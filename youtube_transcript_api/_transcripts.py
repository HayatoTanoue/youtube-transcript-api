...
class FetchedTranscript:
    ...
    def get_summary(self, max_length: int = 200) -> str:
        """Return a truncated summary of the transcript.

        The summary is generated by concatenating all snippet texts and
        truncating the result to ``max_length`` characters without cutting the
        last word.

        :param max_length: maximum length of the returned summary in characters.
        :return: the truncated transcript text
        """

        full_text = " ".join(snippet.text for snippet in self.snippets)
    ...
    def get_word_count(self) -> int:
        ...
        full_text = " ".join(snippet.text for snippet in self.snippets)
        return len(re.findall(r"\w+", full_text))

    def get_duration(self) -> float:
        """Return the total duration of the transcript in seconds."""

        if not self.snippets:
            return 0.0

        start = self.snippets[0].start
        end = self.snippets[-1].start + self.snippets[-1].duration
        return end - start
